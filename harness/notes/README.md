...
| `runs/<run_id>/NOTES.md` | Сводка прогона (пишет `eval.py`) |
| `runs/<run_id>/<task-id>.md` | Стадии, usage, список `out/` (пишет `eval.py`) |
| `runs/<run_id>/analysis.md` | Человеческий разбор (пишем вручную после прогона) |
| [backfill_notes.py](backfill_notes.py) | Восстановить NOTES из уже лежащего `report.json` |

Полные логи и homes не копируем сюда — они в `runs/<run_id>/` рядом с `eval.py`.
После каждого live-eval `eval.py` дублирует краткие NOTES сюда автоматически.
